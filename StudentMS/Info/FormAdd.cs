...
namespace StudentMS.Info
{
    public partial class FormAdd : Form
    {
        ...
        {
            string gender = "";
            if (radioMale.Checked) gender = radioMale.Text;
            else gender = radioFemale.Text;
            string dob = dateTimePickerDOB.Value.ToString("yyyy-MM-dd");

            if (textBoxName.Text == "" || textBoxStudentNo.Text == "" || textBoxDepartment.Text == "")
            {
            ...
            else
            {
                ...
                string sqlquery = String.Format("INSERT INTO [dbo].[Student](StudentNo, StudentName, Gender, Birthday, ClassID, StudentDep) VALUES ('{0}', N'{1}', N'{2}','{3}','{4}', N'{5}')", textBoxStudentNo.Text, textBoxName.Text, gender, dob, comboBoxClass.Text, textBoxDepartment.Text); //SQL语句
                try
                {
                    sqlconn.Open();
                    ...
                    if (count > 0)
                    {
                        MessageBox.Show("您成功新增一位學生", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                    else
                    {
                        MessageBox.Show("學生訊息新增失敗", "提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
